...
export async function processVoiceInput(audioBuffer: Buffer, sourceLanguage: string = "te", sessionId: string = "default") {
  try {
    ...
    try {
        ...
        if (session && (session.stage !== 'complete')) {
          ...
          if (isComplete) {
            ...
            try {
              ...
              const newUser = await storage.createUser(profileData as any);
              console.log("Created new user:", newUser);
              
              responseContent = `[PROFILE_COMPLETE] I've collected all the information needed:
              - Name: ${session.data.name}
              - Type: ${session.data.userType}
              - Location: ${session.data.location}
              - Username: ${session.data.username}
            ...
            let prompt = "Please provide the following information: ";
            
            if (!session.data.name) {
              prompt += "your full name";
            } else if (!session.data.userType) {
              prompt += "whether you are a farmer or a consumer";
            } else if (!session.data.location) {
              prompt += "the name of your village or town";
            } else if (!session.data.username) {
              prompt += "your preferred username for login";
            }
            
            // Use GPT to generate a more natural response
            const messages = [
          ...
          // Regular farming assistant mode
          systemMessage = `You are AgriBuddy, a multilingual agricultural assistant for farmers in India.
            Your primary language is ${sourceLanguage === 'te' ? 'Telugu' : sourceLanguage}.
            Your job is to provide helpful information about:
            1. Crop management, diseases, and best practices
            2. Weather patterns and seasonal advice
            3. Market prices and trends
            4. Soil health and fertilizers
            5. Farming equipment and technology
            
            For each response, follow these rules:
            1. Be concise but informative.
            2. Use simple language appropriate for rural farmers.
            3. When describing images, use [GENERATE_IMAGE: description] tags.
            4. Respond in the same language as the user's message (${sourceLanguage}).
            5. When providing advice, consider low-resource environments.
            6. Include specific, actionable advice when possible.`;
          ...
          const imageMatch = responseContent.match(/\[GENERATE_IMAGE: ([^\]]+)\]/);
          let imageUrl = null;
          ...
          if (imageMatch && imageMatch[1]) {
            try {
              ...
              // Remove the image tag from the response
              responseContent = responseContent.replace(/\[GENERATE_IMAGE: [^\]]+\]/, "");
            } catch (error) {
          ...
          try {
            const audioGeneration = await openai.audio.speech.create({
              model: "tts-1",
              voice: "shimmer",
            ...
            const buffer = await audioGeneration.arrayBuffer();
            audioResponse = Buffer.from(buffer).toString('base64');
          } catch (error) {
            console.error("Failed to generate audio:", error);
            // Generate a simulated audio response as fallback
      ...
      return responseData || {
        response: "I couldn't process your request at this time. Please try again.",
        audioResponse: Buffer.from("Error processing").toString('base64')
      };
      ...
      try {
        fs.unlinkSync(tempFilePath);
        console.log("Temporary audio file deleted after error:", tempFilePath);
      } catch (unlinkError) {
        console.error("Failed to delete temporary file:", unlinkError);
      }
      ...
      return {
        response: "I encountered an error while processing your audio. Please try again.",
        audioResponse: Buffer.from("Error processing").toString('base64')
      };
    }
  } catch (error) {
    ...
    return {
      ...
      audioResponse: Buffer.from("Error occurred").toString('base64')
    };
  }
}